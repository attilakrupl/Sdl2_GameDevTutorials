...
{
    int lSetColorModRes = SDL_SetTextureColorMod( mTexture, aRed, aGreen, aBlue );
    if( DRKR_FAIL( lSetColorModRes ) )
    {
        PRINT_ERROR( SDL_GetError() );
    }
}

void drkrTexture::setBlendMode( SDL_BlendMode aBlending )
{
    int lSetModeResult = SDL_SetTextureBlendMode( mTexture, aBlending );
    if( DRKR_FAIL( lSetModeResult ) )
    {
        PRINT_ERROR( SDL_GetError() );
    }
}

void drkrTexture::setAlpha( uint8_t aAlpha )
{
    int lSetAlphaResult = SDL_SetTextureAlphaMod( mTexture, aAlpha );
    if( DRKR_FAIL( lSetAlphaResult ) )
    {
        PRINT_ERROR( SDL_GetError() );
    }
}
